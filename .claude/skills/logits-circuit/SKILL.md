...
- `LOGITS_EXAMPLE=<document id>` tries **`/preview/example/<id>`** first (least overhead) and falls back to `/preview#data=<encoded document>` while that route does not exist; `LOGITS_CIRCUIT` always uses the data link. `bun .claude/skills/logits-circuit/scripts/link.ts file.json http://localhost:3123` prints that link for manual use.
